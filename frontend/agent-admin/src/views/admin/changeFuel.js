...
import Form from '../../components/forms/fuelForm';
...
class ChangeCar extends Component {
    ...
    add(data) {
        ...
        if (this.props[0].match.params.id){
            fetch(`https://localhost:8080/fuel/update`, {
                method: 'POST',
                headers: {
            ...
            }).then((res) => this.props[0].history.push('/fuels'))


        }else{
            fetch(`https://localhost:8080/fuel/create`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
    ...
    get() {
         if (this.props[0].match.params.id) {
            fetch('https://localhost:8080/fuel/get/' + this.props[0].match.params.id, {
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${localStorage.getItem('token')}`
